...
PRE
"""
#from newmemory import PrioritizedReplayBuffer
...
class DuelingDeepQNetwork(keras.Model):
    def __init__(self, n_actions, fc1_dims, fc2_dims, name, chkpt_dir='tmp/ddqn'):
...
class Agent():
    def __init__(self, lr, gamma, n_actions, epsilon, batch_size, env,
        ...
        self.q_eval = DuelingDeepQNetwork(n_actions, fc1_dims, fc2_dims, name='eval')
        self.q_next = DuelingDeepQNetwork(n_actions, fc1_dims, fc2_dims, name='target')
    ...
    def learn(self):
        ...
        if self.prioritized_replay == False:
            ...
            experience = agent.memory.sample(agent.batch_size, beta=agent.beta_schedule.value(agent.t))
            ...
            q_pred = agent.q_eval(states)
            q_next = agent.q_next(states_)
            # changing q_pred doesn't matter because we are passing states to the train function anyway
            # also, no obvious way to copy tensors in tf2?
            q_target = q_pred.numpy()
            max_actions = tf.math.argmax(agent.q_eval(states_), axis=1)
        
            q_tar =[]
            q_pre =[]
            for idx, terminal in enumerate(dones):
                q_target_ =  rewards[idx] + \
                        agent.gamma*q_next[idx, max_actions[idx]]*(1-int(dones[idx]))
                q_target[idx, actions[idx]] = q_target_
                q_tar.append(q_target_.numpy()[0])
                q_pre_ = q_pred[idx,actions[idx][0]].numpy()
                q_pre.append(q_pre_)
                
            agent.q_eval.train_on_batch(states, q_target,sample_weight=weights)
            agent.epsilon = agent.epsilon - agent.eps_dec if agent.epsilon > \
                        agent.eps_min else agent.eps_min

            agent.learn_step_counter += 1       
            # use Q2 TD error as priority weight
            td_errors = np.array(q_pre) - np.array(q_tar)
            new_priorities = (np.abs(td_errors) + agent.prioritized_replay_eps).flatten()
            agent.memory.update_priorities(batch_idxes, new_priorities)
